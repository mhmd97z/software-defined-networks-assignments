    ...
    try:
	   ...
	   for hostCounter in range(numOfHosts):
        ...
        if(WritingRulesType == "TunnelRules"):
          while True:
             sleep(5)
             counter_id = int(input("1. If you want to count UIDs please press '1'\n2. But if you want to count QOSs please press '2'\n"))
             if  counter_id==1:
                 ind=int(input("which host do you want(1 to 4)?"))
                 ind=ind -1
                  
                 print '\n----- Reading tunnel counters -----'
                 printCounter(p4info_helper, s1, "MyIngress.Idcounter", ind)
                 printCounter(p4info_helper, s2, "MyIngress.Idcounter", ind)
                 printCounter(p4info_helper, s3, "MyIngress.Idcounter", ind)
                 printCounter(p4info_helper, s4, "MyIngress.Idcounter", ind)
                 printCounter(p4info_helper, s5, "MyIngress.Idcounter", ind)
                 printCounter(p4info_helper, s6, "MyIngress.Idcounter", ind)
                 printCounter(p4info_helper, s7, "MyIngress.Idcounter", ind)
             elif counter_id==2:
                 inx=int(input("Please specify quality of service should be counted(0 to 3)?"))
                 print '\n----- Reading tunnel counters -----'
                 printCounter(p4info_helper, s1, "MyIngress.QoScounter", inx)
                 printCounter(p4info_helper, s2, "MyIngress.QoScounter", inx)
                 printCounter(p4info_helper, s3, "MyIngress.QoScounter", inx)
                 printCounter(p4info_helper, s4, "MyIngress.QoScounter", inx)
                 printCounter(p4info_helper, s5, "MyIngress.QoScounter", inx)
                 printCounter(p4info_helper, s6, "MyIngress.QoScounter", inx)
                 printCounter(p4info_helper, s7, "MyIngress.QoScounter", inx)
            else:
                 print("Invalid input")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='./build/part4.p4.p4info.txt')
    parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
                        type=str, action="store", required=False,
                        default='./build/part4.json')
